...
public class UserBookController {
    ...
    // TODO: figure out authorisation and apply it here so my very real users can only view their own user books.

    private final UserBookService service;

    @GetMapping
    ResponseEntity<List<UserBookResponse>> all(@AuthenticationPrincipal AuthUser user) {
        List<UserBookResponse> allBooks = service.getAllUserBooks(user.getId());
        return new ResponseEntity<>(allBooks, HttpStatus.OK);
    }

    @GetMapping("/{id}")
    ResponseEntity<UserBookResponse> one(@AuthenticationPrincipal AuthUser user, @PathVariable Long id) {
        UserBookResponse book = service.getUserBook(id, user.getId());
    ...
    ResponseEntity<UserBookResponse> newUserBook(@AuthenticationPrincipal AuthUser user,
        ...
        UserBookResponse response = service.createUserBook(newUserBook, user.getId());
    ...
    ResponseEntity<UserBookResponse> updateUserBook(@AuthenticationPrincipal AuthUser user,
        ...
        UserBookResponse response = service.updateUserBookById(newUserBook, id, user.getId());
